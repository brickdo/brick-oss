...
import { PagesTree, PageView, PublicAddress } from '../types'

import slugify from '@sindresorhus/slugify'

export const getPageIdWithSlug = (page: PageView) => {
  const pageSlug = slugify(page.name)
  return pageSlug + (pageSlug ? '-' : '') + page.shortId
}

export const getPagePathItem = (page: PageView) => page.customLink || getPageIdWithSlug(page)

export function getPageCanonicalLink(
  pageId: PageView['id'],
