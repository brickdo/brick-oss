...
const BreadcrumbItem = ({ text, link, isApp, className }: BreadcrubItemProps) => {
  const elProps = {
    className: clsx(className, 'mx-1 px-1 breadcrumb-item inline-block whitespace-no-wrap'),
    css: breadcrumbItemCss,
    title: text,
  }
  return link ? (
    isApp ? (
      <NavLink {...elProps} to={link}>
        {text}
      </NavLink>
    ) : (
      <a {...elProps} href={link}>
...
function Breadcrumbs({ items, className, style, isApp }: Props): ReactElement {
  ...
  const breadcrumbItems = items.map(item => (
    <BreadcrumbItem {...item} isApp={isApp} key={item.id} />
  ))
