...
import { RequireAtLeastOne } from '@brick-shared/types'
...
function OptionsTippy(props: OptionsTippyProps): ReactElement {
  ...
  const mapButton = (
    { title, subtitle, icon, onClick, className, children, disabled }: OptionsItemButton,
    key: number,
  ) => (
    <button
      role='menuitem'
...
const style = css`
  :global(.options-tippy) {
    max-width: initial !important;
    font-size: 1rem;
  }
  :global(.options-tippy .tippy-content) {
    padding: 6px 0;
  }
  * :global(.option svg) {
    margin-right: 0.4rem;
